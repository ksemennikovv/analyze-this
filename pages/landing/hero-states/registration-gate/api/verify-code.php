...
$realUserId  = (int)$row['id'];

if ($guestUserId && $guestUserId !== $realUserId) {
    $stmt = $db->prepare('UPDATE analyses SET user_id=? WHERE user_id=?');
    $stmt->bind_param('ii', $realUserId, $guestUserId);
    $stmt->execute(); $stmt->close();

    $stmt = $db->prepare('UPDATE analysis_messages SET user_id=? WHERE user_id=?');
    $stmt->bind_param('ii', $realUserId, $guestUserId);
    $stmt->execute(); $stmt->close();

    $stmt = $db->prepare("DELETE FROM users WHERE id=? AND email LIKE 'guest\\_%@nirva.ai'");
...
unset($_SESSION['guest_user_id'], $_SESSION['guest_analysis_id'], $_SESSION['guest_analysis_title']);
